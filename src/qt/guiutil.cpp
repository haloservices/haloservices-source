...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("halocoin"))
        return false;
...
{
    // Convert halocoin:// to halocoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("halocoin://"))
    {
        uri.replace(0, 12, "halocoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "HaloCoin.lnk";
}
...
{
    return GetAutostartDir() / "halocoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("HaloCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  halocoin-qt [" + tr("command-line options") + "]                     " + "\n";
